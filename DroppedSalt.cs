...
internal static class DroppedSalt
{
    ...
    {
        if (__instance.droppedXPArea != GameSessionMgr.gameSession.currentArea) return false;
        var screenLoc = ScrollManager.GetScreenLoc(__instance.droppedXPVec, 0);
        if (screenLoc.X <= -100f || screenLoc.Y <= -100f || screenLoc.X >= ScrollManager.screenSize.X + 100f ||
        ...
        if (IsLocalCoopModeMethod != null) isCoopMode = (bool)IsLocalCoopModeMethod.Invoke(null, null);
        ...
        for (var i = 0; i < 4; i++)
        {
            ...
            Textures.tex[ParticleManager.spritesTexIdx].Draw(screenLoc, 9 + i,
                new Vector2(1f, 0.5f) * ScrollManager.cannedDepth[0] * num, 0f, r, g, b, num2 * 4f);
        }
    ...
    private static IEnumerable<CodeInstruction> XpWispDrawTranspiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        foreach (var instr in codes)
        {
            ...
            if (instr.IsLdloc())
            {
                ...
                if (currentIdx == localR)
                    component = 0;
                else if (currentIdx == localG)
                    component = 1;
            ...
            yield return new CodeInstruction(OpCodes.Call,
                AccessTools.Method(typeof(DroppedSalt), nameof(GetCorrectedColor)));
        }
    }
